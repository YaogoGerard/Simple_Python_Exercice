# Simple Python Exercises

This project contains several small Python programs you can run in the terminal.

## Included Programs

- **Guess The Number** (`Guess_The_Number/GTN.py`)  
  The program chooses a random number and the user must guess it.

- **Hangman** (`Hangman/hangman.py`)  
  Hangman game: the user guesses a word letter by letter with a limited number of attempts.

- **Madlibs** (`Madlibs/madlib.py`)  
  The user enters words (adjective, verbs, name) to generate a fun story.

- **Rock Paper Scissors** (`Rock_Paper_Scissors/RPS.py`)  
  Rock–paper–scissors game against the computer.

- **Tic Tac Toe** (`Tic_Tac_Toe/game.py` + `Tic_Tac_Toe/player.py`)  
  Tic-tac-toe game against a computer that plays randomly.

## Running
Each program is launched from its folder with `python filename.py`.
